...
//function that will have element and ivdata input and it will check if there is a textarea in the element and if there is it will add the text from the divData
/**
 * Adds text to a textarea within the given element if it exists.
 *
 * @param {jQuery} element - The jQuery element to search for a textarea.
 * @param {Object} divData - The data object containing the text to be added.
 * @param {string} divData.text - The text to set in the textarea.
 */
function addInputText(element, divData) {
    if (element.find('textarea').length) {
        element.find('textarea').val(divData.text);
    }
}
